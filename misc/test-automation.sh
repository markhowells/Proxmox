...
TEMP_SCRIPT=$(mktemp)
sed 's|https://raw.githubusercontent.com/markhowells/Proxmox/main/misc/build.func|file://'"${SCRIPT_DIR}"'/misc/auto-build.func|g' "$ORIGINAL_SCRIPT" > "$TEMP_SCRIPT"
...
test_script() {
    ...
    TEMP_SCRIPT=$(mktemp)
    sed 's|source <(curl -fsSL https://raw.githubusercontent.com/markhowells/Proxmox/main/misc/build.func)|source misc/auto-build.func|g' "${script_path}" > "$TEMP_SCRIPT"
